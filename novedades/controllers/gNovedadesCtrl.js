...
function generar() {
    ...
    if (error == 0) {
        var parametros = {
            "fec_desde": fec_desde, "fec_hasta": fec_hasta, "usuario": usuario
        };
        $.ajax({
            data: parametros,
            ...
            success: function (response) {
                ...
                if (resp.length > 0) {
                    ...
                    else g.crearGraficaTorta(resp, 450, 'grafica', 'nov1', true, true, 'top', 'col-xs-6', 'Novedades por Status');
                    g2.borrarGrafica('grafica', 'nov2', () => {
                        novStatusDet_inic(resp[0].codigo, resp[0].titulo);
                    });
                    resp.forEach(function (d, i) { novStatusDet(d.codigo, d.titulo); });
                } else {
                    $('.brs').hide();
                    $('#sin_data').show();
                    $('#grafica').hide();
...
function iniciar() {
    ...
    if (error == 0) {
        var parametros = { "fec_desde": fec_desde, "fec_hasta": fec_hasta, "usuario": usuario };
...
function novStatusDet(status, titulo) {
    ...
    if (error == 0) {
        var parametros = {
            "fec_desde": fec_desde, "fec_hasta": fec_hasta, "status": status
        };
        $.ajax({
            data: parametros,
